// Lista de créditos
creditos = [
	"Obrigado por jogar",
    "Ferramentas usadas: GameMaker, Piskel para designs.",
    "Instituição: Centro Universitário Santa Terezinha",
	"Curso: Sistemas de Informação, 7° Período - Noturno",
	"Disciplina: Seminário Temático, Desenvolvimento de jogos, Programação Móvel, Gerenciamento de Projetos",
    "Ano: 2025",
	"Programação: Gabriel Viana de Carvalho",
	"Game Design: Thiago Henrique Coelho da Silva",
	"Sound Design: Yan Patrick Rodrigues de Moraes",
	"Game Testers: Letícia Ferreira do Nascimento",
	"Documentação: Lucas Gabriel Vale Gomes",
	
	"Créditos de Sprites:",
	"BananaMan (Jogador):sprite sheet feita por 'Hildemuz' e modificada", 
	"Tomate (Inimigo):sprite sheet feito por 'Defadaj'",
	"Cenoura (Inimigo):sprite sheet feito por 'Defadaj'",
	"Backgrounds:",
	"Tilesets: Cute Forest feito por 'Nauris, Namatnieks'",

	"Créditos de Músicas e sons:",
	"Música de inicio: Freesound 8 bit game loop 002 simple mix 2 long 120 bpm.wav by josefpres",
	"Música de Batalha: Freesound - Complex Ham by tripjazz",
	"Música de Venceu: Freesound - music game, win or high score.wav by xtrgamr",
	"Dano ao jogador: Feesound - Hit_01.wav by LittleRobotSoundFactory",
	"Ataque jogador: Freesound - Tri Whoosh DPCM by EtherAudio",
	"Botões: Freesound - eventsounds4 f3.mp3 by B_Lamerichs",
	"Seleção de Habilidade: Freesound - Menu_Select by SomeUsername0",
	"Ataque do tomate: Freesound - Jembay Hit 8 Rim.wav by carlmartin"

];

// Controle de rolagem
scroll_y = 0;
dragging = false;
drag_start_y = 0;
scroll_start_y = 0;
